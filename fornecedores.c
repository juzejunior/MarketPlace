#include "include/fornecedores.h"
/* 
 * File:   fornecedores.c
 * Author: junior(Diôgo)
 *
...
int cadastrarFornecedor(Fornecedores **fornecedor, char nomeFornecedor[], char nomeProduto[])
{
	...
	/*novo_fornecedor->prox = *fornecedor;
	*fornecedor = novo_fornecedor;*/
...
{
	Fornecedores *f;
	Produtos *p, *anterior, *liberar;
	
	f = *fornecedor;
	
	while(f != NULL)
	{
		/*se o fornecedor existir verifique a existencia do produto e se existir remova-o*/
		if(strcmp(f->nome, nomeFornecedor) == 0)
		{
			p = f->produto;
			anterior = p;
			...
			while(p != NULL)
			{
				if(strcmp(p->nome,nomeProduto) == 0)
				{
					if(p == f->produto)//caso seja o primeiro ponteiro
					{
						liberar = p;
						f->produto = p->prox;
						free(liberar);
