...
int inserirFornecedor()
{
	...
	do{
	   ...
	   if(existeFornecedor(fornecedor.nome) == 0)
	   {
		    printf(" E-mail: ");
			scanf(" %[^\n]s", fornecedor.email);
			printf(" Telefone: ");
...
void exibirProdutosFornecedores()
{
	...
	do{
		...
		if(existeFornecedor(nomeFornecedor) != 0)
		{
			...
			printf("\n Fornecedor: %s\n\n", f.nome);
			printf("	Produtos: \n");
			while(fread(&p,sizeof(Produto),1, file_produtos_forne) == 1)
			{
...
void excluirProdutoFornecedor()
{
	...
	do{
	  ...
	  {
		  while(fread(&aux,sizeof(Produto),1, file_produto) == 1)
		  {
			 if(strcmp(aux.nome, p.nome) == 0 && strcmp(aux.nomeFornecedor, p.nomeFornecedor) == 0) 
			 {
				 encontrou = 1;
			 }else{
				 fwrite(&aux, sizeof(Produto),1, file_aux);
			 }
		  }
